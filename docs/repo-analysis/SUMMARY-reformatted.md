...
15	VRAMPeakMonitor 峰值监控工具（DiffBIR 显存峰值追踪器）	DiffBIR	P1

二、帧间一致性 / 时序处理（12 个仓库涉及）
#	建议	来源仓库	优先级
...
12	截断因果历史模型（Turtle num_frames_tocache 精确控制时序依赖范围）	Turtle	P1

三、多引擎架构 / 引擎调度（8 个仓库涉及）
#	建议	来源仓库	优先级
...
8	子进程引擎调用（upscayl spawnUpscayl() 进程封装模式）	upscayl	P2

四、扩散调度 / CFG / 采样器（10 个仓库涉及）
#	建议	来源仓库	优先级
...
10	Flow Matching 调度器（HunyuanVideo FlowMatchDiscreteScheduler + sd3_time_shift）	HunyuanVideo	P2

五、后处理 / 颜色校正 / 质量增强（10 个仓库涉及）
#	建议	来源仓库	优先级
...
10	多步放大策略（clarity-upscaler 最多 3 次迭代放大避免质量下降）	clarity-upscaler	P2

六、模型架构 / DiT 优化（8 个仓库涉及）
#	建议	来源仓库	优先级
...
8	多模态融合架构（EvTexture 事件纹理提取 + 帧特征融合）	EvTexture	P3
...
7	分级退化处理（STAR light_deg/heavy_deg 不同退化程度预设参数）	STAR	P2

八、WebUI / 用户交互（6 个仓库涉及）
#	建议	来源仓库	优先级
...
6	文件拖拽支持（upscayl 图片/文件夹拖拽添加）	upscayl	P2
...
5	Gradient Checkpointing（RVRT 逐层选择是否使用 checkpoint）	RVRT	P2
...
6	模型自描述属性（waifu2x w2nn_* 内嵌元数据，推理时自动适配）	waifu2x	P2
7	Python 绑定直调（Anime4KCPP pybind11 零拷贝 NumPy 传入）	Anime4KCPP	P2
8	Hydra 配置管理（Fast-SRGAN YAML + CLI 覆盖）	Fast-SRGAN	P3

十一、专用引擎 / 场景扩展（7 个仓库涉及）
#	建议	来源仓库	优先级
...
7	视频 Inpainting 引擎（ProPainter 双向传播 + Temporal Sparse Transformer）	ProPainter	P3

十二、GPU / 硬件兼容性（5 个仓库涉及）
#	建议	来源仓库	优先级
...
5	GPU 枚举和兼容性检测（Waifu2x-Extension-GUI 各引擎 GPU 检测统一）	Waifu2x-Extension-GUI	P1
...
VEnhancer 依赖 fairscale 梯度检查点，可能与 SeedVR2 的 PyTorch 版本冲突	VEnhancer
