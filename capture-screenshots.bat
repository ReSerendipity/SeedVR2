...
title SeedVR2 Screenshot Capture

set "ROOT=%~dp0"
set "TESTS_DIR=%ROOT%tests"
set "BASE_URL=http://127.0.0.1:7870"
set "HEALTH=%BASE_URL%/api/system/health"

echo ============================================
echo   SeedVR2 - Auto Screenshot Capture
echo ============================================
echo.
...
echo [INFO] Starting SeedVR2 server ^(degraded mode is OK - UI screenshots do not need GPU^) ...
start "SeedVR2 Server" /min "%PYTHON_CMD%" "%ROOT%bin\clean_launch.py"
...
echo         Check logs\app.log. If there is no NVIDIA GPU, the app should still
echo         start in degraded mode; if it exited, see the log for details.
pause
exit /b 1
...
echo ============================================
echo.
echo NOTE: The SeedVR2 server window may still be open ^(minimized^).
echo       Close it manually if you no longer need it.
pause
exit /b !RC!

:check_health
REM Uses curl (shipped with Windows 10 1803+) to avoid PowerShell variable
REM quoting issues. Exit 0 = HTTP 200 (ready), non-zero = not ready.
