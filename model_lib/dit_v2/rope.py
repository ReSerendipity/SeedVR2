...
class NaMMRotaryEmbedding3d(MMRotaryEmbeddingBase):
    ...
    def get_freqs(
        ...
        """为当前 batch 的实际尺寸生成 RoPE 频率（与 ComfyUI 实现对齐）。

        相比固定生成最大尺寸 (1024, 128, 128)，按 batch 实际最大尺寸生成，
        显著降低峰值显存与生成耗时。
        """
        max_temporal = 0
        max_height = 0
        max_width = 0
        max_txt_len = 0
        for (f, h, w), l in zip(vid_shape.tolist(), txt_shape[:, 0].tolist(), strict=False):
        ...
        target_device = vid_shape.device
